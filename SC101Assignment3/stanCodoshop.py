...
def get_pixel_dist(pixel, red, green, blue):
    ...
    """
    color_d = ((red - pixel.red)**2 + (green - pixel.green)**2 + (blue - pixel.blue)**2)**0.5
    return color_d
...
def get_average(pixels):
    ...
    l_avg = [red_avg, green_avg, blue_avg]
    return l_avg
...
def get_best_pixel(pixels):
    ...
    """
    color_d0 = 442   # Max color_distance = ((225**2)+(225**2)+(225**2))**0.5 = 441.6
    l_avg = get_average(pixels)
    for i in range(len(pixels)):
        color_d1 = get_pixel_dist(pixels[i], l_avg[0], l_avg[1], l_avg[2])
        if color_d1 < color_d0:
            best1 = pixels[i]
            color_d0 = color_d1
    return best1
...
def solve(images):
    ...
    for y in range(height):
        for x in range(width):
            pixels = []
            for image in images:  # 但我不懂他在哪邊控制找到別張照片同一個位置的...就是把他硬做成一個list,然後好像就成功了?
